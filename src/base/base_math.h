...
static inline float DeterminantM4(Mat4 Matrix) {
  ...
  Vec3 B10 = SubV3(MulV3F(Matrix.Columns[0].XYZ, Matrix.Columns[1].W),
                   MulV3F(Matrix.Columns[1].XYZ, Matrix.Columns[0].W));
  Vec3 B32 = SubV3(MulV3F(Matrix.Columns[2].XYZ, Matrix.Columns[3].W),
                   MulV3F(Matrix.Columns[3].XYZ, Matrix.Columns[2].W));

  return DotV3(C01, B32) + DotV3(C23, B10);
}

COVERAGE(InvGeneralM4, 1)
// Returns a general-purpose inverse of an Mat4. Note that special-purpose
...
static inline Mat4 InvGeneralM4(Mat4 Matrix) {
  ...
  Mat4 Result;
  Result.Columns[0] = V4V(AddV3(Cross(Matrix.Columns[1].XYZ, B32),
                                MulV3F(C23, Matrix.Columns[1].W)),
                          -DotV3(Matrix.Columns[1].XYZ, C23));
  Result.Columns[1] = V4V(SubV3(Cross(B32, Matrix.Columns[0].XYZ),
                                MulV3F(C23, Matrix.Columns[0].W)),
                          +DotV3(Matrix.Columns[0].XYZ, C23));
  Result.Columns[2] = V4V(AddV3(Cross(Matrix.Columns[3].XYZ, B10),
                                MulV3F(C01, Matrix.Columns[3].W)),
                          -DotV3(Matrix.Columns[3].XYZ, C01));
  Result.Columns[3] = V4V(SubV3(Cross(B10, Matrix.Columns[2].XYZ),
                                MulV3F(C01, Matrix.Columns[2].W)),
                          +DotV3(Matrix.Columns[2].XYZ, C01));
...
static inline Mat4 Orthographic_RH_NO(float Left,
                                      float Right,
                                      float Bottom,
                                      float Top,
                                      float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline Mat4 Orthographic_RH_ZO(float Left,
                                      float Right,
                                      float Bottom,
                                      float Top,
                                      float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline Mat4 Orthographic_LH_NO(float Left,
                                      float Right,
                                      float Bottom,
                                      float Top,
                                      float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline Mat4 Orthographic_LH_ZO(float Left,
                                      float Right,
                                      float Bottom,
                                      float Top,
                                      float Near,
...
static inline Mat4 Perspective_RH_NO(float FOV,
                                     float AspectRatio,
                                     float Near,
                                     float Far) {
...
static inline Mat4 Perspective_RH_ZO(float FOV,
                                     float AspectRatio,
                                     float Near,
                                     float Far) {
...
static inline Mat4 Perspective_LH_NO(float FOV,
                                     float AspectRatio,
                                     float Near,
                                     float Far) {
...
static inline Mat4 Perspective_LH_ZO(float FOV,
                                     float AspectRatio,
                                     float Near,
                                     float Far) {
...
static inline Quat M4ToQ_RH(Mat4 M) {
  ...
  if (M.Elements[2][2] < 0.0f) {
    if (M.Elements[0][0] > M.Elements[1][1]) {
      ...
      q = Q(T, M.Elements[0][1] + M.Elements[1][0],
      ...
      T = 1 - M.Elements[0][0] + M.Elements[1][1] - M.Elements[2][2];
      q = Q(M.Elements[0][1] + M.Elements[1][0], T,
            M.Elements[1][2] + M.Elements[2][1],
            M.Elements[2][0] - M.Elements[0][2]);
    }
  } else {
    if (M.Elements[0][0] < -M.Elements[1][1]) {
      ...
      T = 1 - M.Elements[0][0] - M.Elements[1][1] + M.Elements[2][2];
      q = Q(M.Elements[2][0] + M.Elements[0][2],
      ...
      T = 1 + M.Elements[0][0] + M.Elements[1][1] + M.Elements[2][2];
      q = Q(M.Elements[1][2] - M.Elements[2][1],
            M.Elements[2][0] - M.Elements[0][2],
...
static inline Quat M4ToQ_LH(Mat4 M) {
  ...
  if (M.Elements[2][2] < 0.0f) {
    ...
    if (M.Elements[0][0] < -M.Elements[1][1]) {
      ...
      T = 1 + M.Elements[0][0] + M.Elements[1][1] + M.Elements[2][2];
      q = Q(M.Elements[2][1] - M.Elements[1][2],
            M.Elements[0][2] - M.Elements[2][0],
...
static inline Vec4 operator-(Vec4 In) {
  ...
  #define Div(A, B)          \
    _Generic((B),            \
        ...
        Mat4: DivM4,         \
        Quat: DivQ,          \
        default: _Generic((A), Vec2: DivV2, Vec3: DivV3, Vec4: DivV4))(A, B)

  #define Len(A) _Generic((A), Vec2: LenV2, Vec3: LenV3, Vec4: LenV4)(A)

  #define LenSqr(A) \
    _Generic((A), Vec2: LenSqrV2, Vec3: LenSqrV3, Vec4: LenSqrV4)(A)

  #define Norm(A) _Generic((A), Vec2: NormV2, Vec3: NormV3, Vec4: NormV4)(A)

  #define Dot(A, B) _Generic((A), Vec2: DotV2, Vec3: DotV3, Vec4: DotV4)(A, B)

  #define Lerp(A, T, B)                                                        \
    _Generic((A), float: Lerp, Vec2: LerpV2, Vec3: LerpV3, Vec4: LerpV4)(A, T, \
                                                                         B)

  #define Eq(A, B) _Generic((A), Vec2: EqV2, Vec3: EqV3, Vec4: EqV4)(A, B)

  #define Transpose(M) \
    _Generic((M), Mat2: TransposeM2, Mat3: TransposeM3, Mat4: TransposeM4)(M)
  ...
  #define InvGeneral(M) \
    _Generic((M), Mat2: InvGeneralM2, Mat3: InvGeneralM3, Mat4: InvGeneralM4)(M)
