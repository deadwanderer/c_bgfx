...
// Dummy macros for when test framework is not present.
  ...
  #warning \
      "HANDMADE_MATH_NO_SSE is deprecated, use HANDMADE_MATH_NO_SIMD instead"
...
extern "C" {
    ...
    !defined(HANDMADE_MATH_USE_TURNS) && !defined(HANDMADE_MATH_USE_RADIANS)
  #define HANDMADE_MATH_USE_RADIANS
#endif
...
static inline float HMM_DeterminantM4(HMM_Mat4 Matrix) {
  ...
  HMM_Vec3 B10 =
      HMM_SubV3(HMM_MulV3F(Matrix.Columns[0].XYZ, Matrix.Columns[1].W),
                HMM_MulV3F(Matrix.Columns[1].XYZ, Matrix.Columns[0].W));
  HMM_Vec3 B32 =
      HMM_SubV3(HMM_MulV3F(Matrix.Columns[2].XYZ, Matrix.Columns[3].W),
                HMM_MulV3F(Matrix.Columns[3].XYZ, Matrix.Columns[2].W));

  return HMM_DotV3(C01, B32) + HMM_DotV3(C23, B10);
}

COVERAGE(HMM_InvGeneralM4, 1)
// Returns a general-purpose inverse of an HMM_Mat4. Note that special-purpose
...
static inline HMM_Mat4 HMM_InvGeneralM4(HMM_Mat4 Matrix) {
  ...
  HMM_Mat4 Result;
  Result.Columns[0] = HMM_V4V(HMM_AddV3(HMM_Cross(Matrix.Columns[1].XYZ, B32),
                                        HMM_MulV3F(C23, Matrix.Columns[1].W)),
                              -HMM_DotV3(Matrix.Columns[1].XYZ, C23));
  Result.Columns[1] = HMM_V4V(HMM_SubV3(HMM_Cross(B32, Matrix.Columns[0].XYZ),
                                        HMM_MulV3F(C23, Matrix.Columns[0].W)),
                              +HMM_DotV3(Matrix.Columns[0].XYZ, C23));
  Result.Columns[2] = HMM_V4V(HMM_AddV3(HMM_Cross(Matrix.Columns[3].XYZ, B10),
                                        HMM_MulV3F(C01, Matrix.Columns[3].W)),
                              -HMM_DotV3(Matrix.Columns[3].XYZ, C01));
  Result.Columns[3] = HMM_V4V(HMM_SubV3(HMM_Cross(B10, Matrix.Columns[2].XYZ),
                                        HMM_MulV3F(C01, Matrix.Columns[2].W)),
                              +HMM_DotV3(Matrix.Columns[2].XYZ, C01));
...
static inline HMM_Mat4 HMM_Orthographic_RH_NO(float Left,
                                              float Right,
                                              float Bottom,
                                              float Top,
                                              float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline HMM_Mat4 HMM_Orthographic_RH_ZO(float Left,
                                              float Right,
                                              float Bottom,
                                              float Top,
                                              float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline HMM_Mat4 HMM_Orthographic_LH_NO(float Left,
                                              float Right,
                                              float Bottom,
                                              float Top,
                                              float Near,
...
// coordinates of their respective clipping planes. Near and Far specify the
// distances to the near and far clipping planes.
static inline HMM_Mat4 HMM_Orthographic_LH_ZO(float Left,
                                              float Right,
                                              float Bottom,
                                              float Top,
                                              float Near,
...
static inline HMM_Mat4 HMM_Perspective_RH_NO(float FOV,
                                             float AspectRatio,
                                             float Near,
                                             float Far) {
...
static inline HMM_Mat4 HMM_Perspective_RH_ZO(float FOV,
                                             float AspectRatio,
                                             float Near,
                                             float Far) {
...
static inline HMM_Mat4 HMM_Perspective_LH_NO(float FOV,
                                             float AspectRatio,
                                             float Near,
                                             float Far) {
...
static inline HMM_Mat4 HMM_Perspective_LH_ZO(float FOV,
                                             float AspectRatio,
                                             float Near,
                                             float Far) {
...
static inline HMM_Mat4 _HMM_LookAt(HMM_Vec3 F,
                                   HMM_Vec3 S,
                                   HMM_Vec3 U,
                                   HMM_Vec3 Eye) {
  HMM_Mat4 Result;
...
static inline HMM_Mat4 HMM_LookAt_RH(HMM_Vec3 Eye,
                                     HMM_Vec3 Center,
                                     HMM_Vec3 Up) {
...
static inline HMM_Mat4 HMM_LookAt_LH(HMM_Vec3 Eye,
                                     HMM_Vec3 Center,
                                     HMM_Vec3 Up) {
...
static inline HMM_Quat HMM_MulQ(HMM_Quat Left, HMM_Quat Right) {
  ...
  Result.NEON =
      vmulq_f32(Right3210, vmulq_f32(vdupq_laneq_f32(Left.NEON, 0), FirstSign));
  float32x4_t SecondSign = {1.0f, 1.0f, -1.0f, -1.0f};
  Result.NEON = vfmaq_f32(Result.NEON, Right2301,
                          vmulq_f32(vdupq_laneq_f32(Left.NEON, 1), SecondSign));
  float32x4_t ThirdSign = {-1.0f, 1.0f, 1.0f, -1.0f};
  Result.NEON = vfmaq_f32(Result.NEON, Right1032,
...
static inline HMM_Quat _HMM_MixQ(HMM_Quat Left,
                                 float MixLeft,
                                 HMM_Quat Right,
                                 float MixRight) {
  HMM_Quat Result;
...
static inline HMM_Quat HMM_SLerp(HMM_Quat Left, float Time, HMM_Quat Right) {
  ...
  /* NOTE(lcf): Use Normalized Linear interpolation when vectors are roughly not
   * L.I. */
...
static inline HMM_Quat HMM_M4ToQ_RH(HMM_Mat4 M) {
  ...
  if (M.Elements[2][2] < 0.0f) {
    if (M.Elements[0][0] > M.Elements[1][1]) {
      ...
      Q = HMM_Q(T, M.Elements[0][1] + M.Elements[1][0],
      ...
      T = 1 - M.Elements[0][0] + M.Elements[1][1] - M.Elements[2][2];
      Q = HMM_Q(M.Elements[0][1] + M.Elements[1][0], T,
                M.Elements[1][2] + M.Elements[2][1],
                M.Elements[2][0] - M.Elements[0][2]);
    }
  } else {
    if (M.Elements[0][0] < -M.Elements[1][1]) {
      ...
      T = 1 - M.Elements[0][0] - M.Elements[1][1] + M.Elements[2][2];
      Q = HMM_Q(M.Elements[2][0] + M.Elements[0][2],
                M.Elements[1][2] + M.Elements[2][1], T,
                M.Elements[0][1] - M.Elements[1][0]);
    } else {
      ASSERT_COVERED(HMM_M4ToQ_RH);

      T = 1 + M.Elements[0][0] + M.Elements[1][1] + M.Elements[2][2];
      Q = HMM_Q(M.Elements[1][2] - M.Elements[2][1],
                M.Elements[2][0] - M.Elements[0][2],
                M.Elements[0][1] - M.Elements[1][0], T);
    }
  }
...
static inline HMM_Quat HMM_M4ToQ_LH(HMM_Mat4 M) {
  ...
  if (M.Elements[2][2] < 0.0f) {
    ...
    if (M.Elements[0][0] < -M.Elements[1][1]) {
      ...
      T = 1 + M.Elements[0][0] + M.Elements[1][1] + M.Elements[2][2];
      Q = HMM_Q(M.Elements[2][1] - M.Elements[1][2],
                M.Elements[0][2] - M.Elements[2][0],
                M.Elements[1][0] - M.Elements[0][2], T);
    }
  }
...
static inline HMM_Vec3 HMM_RotateV3AxisAngle_LH(HMM_Vec3 V,
                                                HMM_Vec3 Axis,
                                                float Angle) {
...
static inline HMM_Vec3 HMM_RotateV3AxisAngle_RH(HMM_Vec3 V,
                                                HMM_Vec3 Axis,
                                                float Angle) {
